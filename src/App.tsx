    ...
    <div className={`min-h-screen ${themeClasses.background} ${themeClasses.text} relative overflow-hidden transition-all duration-700`}>
      ...
      <section id="projects" className={`py-8 sm:py-12 md:py-16 px-4 sm:px-6 relative z-10`}>
        <div className="max-w-5xl mx-auto">
          ...
          {/* SoundAdapt Project */}
          {t.projects.soundAdapt && (
            <div className={`relative p-6 sm:p-8 lg:p-10 rounded-3xl ${themeClasses.cardBg} border ${themeClasses.cardBorder} backdrop-blur-sm mb-6 mx-auto max-w-5xl`}>
              <div className={`absolute inset-0 rounded-3xl ${isDarkTheme ? 'bg-gradient-to-br from-teal-500/5 to-cyan-500/5' : 'bg-gradient-to-br from-teal-100/50 to-cyan-100/50'}`}></div>
              <div className="relative z-10">
                <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                  <div className="flex-1">
                    <h3 className="text-3xl sm:text-4xl font-bold bg-gradient-to-r from-teal-400 to-cyan-500 bg-clip-text text-transparent mb-2">
                      {t.projects.soundAdapt.title}
                    </h3>
                    <p className={`text-lg sm:text-xl ${themeClasses.textSecondary} mb-3`}>
                      {t.projects.soundAdapt.subtitle}
                    </p>
                    <p className={`${themeClasses.textSecondary} leading-relaxed mb-4 sm:mb-5 text-sm sm:text-base`}>
                      {t.projects.soundAdapt.description}
                    </p>
                    <div className="mb-4 sm:mb-5">
                      <h4 className={`text-base sm:text-lg font-semibold ${themeClasses.text} mb-2 sm:mb-3`}>{t.projects.technologies}</h4>
                      <div className="flex flex-wrap gap-2">
                        {t.projects.soundAdapt.techList.map((tech, index) => (
                          <span key={index} className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium ${isDarkTheme ? 'bg-slate-700/50 text-teal-300' : 'bg-teal-50 text-teal-700'} border ${isDarkTheme ? 'border-teal-500/30' : 'border-teal-200'}`}>
                            {tech}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="flex flex-col sm:flex-row lg:flex-col gap-3 sm:gap-4 lg:min-w-[200px]">
                    ...
                    >
                      {t.projects.soundAdapt.visitWebsite}
                      <ArrowRight className="w-4 h-4 ml-2" />
                    </a>
                    <a
                      href="https://chromewebstore.google.com/detail/soundadapt/dcmjdbbdefhimbocfdohphldoecbkgdc"
                    ...
                    >
                      {t.projects.soundAdapt.downloadExtension}
                      <Download className="w-4 h-4 ml-2" />
                    </a>
                  </div>
                </div>
